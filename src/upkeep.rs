...
fn resolve_under_root(root: &Path, rel: &Path) -> Option<PathBuf> {
...
fn norm_wikilink_target(wiki: &Path, target: &str) -> Option<PathBuf> {
...
fn norm_md_link(wiki: &Path, src_file: &Path, target: &str) -> Option<PathBuf> {
    let t = target.trim();
