...
NEWTON-0025 - Each command and subcommand MUST have a proper descriptive help message (about/long_about as needed) including at least one example (e.g. in after_help); `init`, `ingest`, `materialize`, and `check` MUST document optional `-C` / `--wiki-root` (default cwd) where applicable
